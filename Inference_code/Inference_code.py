...
mode_arg.add_argument('--title', type=str, default="inception_resnet_v2")
...
mode_arg.add_argument('--meta_file_path_infer', type=str, default="D:\\HDL\\Project\\Classification\\191209_0.8PAD\\CheckPoint\\AUG_ROT90_CROP0.7\\0199-0133\\CLASSIFICATION.meta")
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        for j in range(int(np.ceil(len(test_data)/config.batch_size))):
            ...
            ##### mobilenet #####
            predictions_o = sess.run([predictions], {input : batch_in, keep_prob : 1, is_training : False}) # test set 모두 사용하면서 cal_accuracy 평균은 틀린값이 됨.
            #predictions_o = sess.run([predictions], {input : batch_in, is_training : False}) # test set 모두 사용하면서 cal_accuracy 평균은 틀린값이 됨.
            #####################
